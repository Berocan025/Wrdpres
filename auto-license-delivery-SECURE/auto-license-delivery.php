...
class AutoLicenseDelivery {
    ...
    private function init_hooks() {
        ...
        // Özelleştirilebilir geliştirici bilgileri için admin ayarları
        add_action('admin_menu', array($this, 'add_customization_menu'), 11);
    }
    ...
    public function add_customization_menu() {
        add_submenu_page(
            'auto-license-delivery',
            'Frontend Özelleştirme - BERAT K',
    ...
    public function customization_page() {
        ...
        if (isset($_POST['save_frontend_settings'])) {
            if (wp_verify_nonce($_POST['ald_custom_nonce'], 'ald_save_frontend')) {
                update_option('ald_frontend_dev_name', sanitize_text_field($_POST['frontend_dev_name']));
                update_option('ald_frontend_dev_phone', sanitize_text_field($_POST['frontend_dev_phone']));
                update_option('ald_frontend_dev_whatsapp', esc_url_raw($_POST['frontend_dev_whatsapp']));
                update_option('ald_frontend_dev_email', sanitize_email($_POST['frontend_dev_email']));
                update_option('ald_frontend_show_custom', isset($_POST['frontend_show_custom']) ? 1 : 0);
                
                echo '<div class="notice notice-success"><p>✅ Frontend ayarları kaydedildi!</p></div>';
            }
        }
        
        // Mevcut ayarları al
        $frontend_dev_name = get_option('ald_frontend_dev_name', '');
        $frontend_dev_phone = get_option('ald_frontend_dev_phone', '');
        $frontend_dev_whatsapp = get_option('ald_frontend_dev_whatsapp', '');
        $frontend_dev_email = get_option('ald_frontend_dev_email', '');
        $frontend_show_custom = get_option('ald_frontend_show_custom', 0);
        
        ?>
        <div class="ald-dashboard">
            <h1 style="color: #333; margin-bottom: 30px;">🎨 Frontend Özelleştirme</h1>
            
            <div style="background: #fff3cd; border: 1px solid #ffeaa7; border-radius: 8px; padding: 15px; margin-bottom: 30px;">
                <h3 style="color: #856404; margin: 0 0 10px 0;">ℹ️ Önemli Bilgi</h3>
                <p style="color: #856404; margin: 0;">Bu ayarlar <strong>sadece müşteri tarafında</strong> (sipariş sayfaları ve e-postalar) görünür. Admin paneli ve plugin kodları <strong>BERAT K</strong> olarak korunmaktadır.</p>
            </div>
            
            <div class="ald-card">
                <div class="ald-card-header">
                    🛠️ Müşteri Tarafı Geliştirici Bilgileri
                </div>
                <div class="ald-card-body">
                    <form method="post" action="">
                        <?php wp_nonce_field('ald_save_frontend', 'ald_custom_nonce'); ?>
                        
                        <div class="ald-form-group">
                            <label class="ald-form-label">
                                <input type="checkbox" name="frontend_show_custom" value="1" <?php checked($frontend_show_custom, 1); ?> style="margin-right: 8px;">
                                Özel geliştirici bilgilerini kullan (kapalı ise BERAT K görünür)
                            </label>
                        </div>
                        
                        <div class="ald-form-group">
                            <label class="ald-form-label">Geliştirici/Firma Adı:</label>
                            <input type="text" name="frontend_dev_name" class="ald-form-control" value="<?php echo esc_attr($frontend_dev_name); ?>" placeholder="Firma Adınız">
                        </div>
                        
                        <div class="ald-form-group">
                            <label class="ald-form-label">Telefon Numarası:</label>
                            <input type="text" name="frontend_dev_phone" class="ald-form-control" value="<?php echo esc_attr($frontend_dev_phone); ?>" placeholder="0XXX XXX XX XX">
                        </div>
                        
                        <div class="ald-form-group">
                            <label class="ald-form-label">WhatsApp Linki:</label>
                            <input type="url" name="frontend_dev_whatsapp" class="ald-form-control" value="<?php echo esc_attr($frontend_dev_whatsapp); ?>" placeholder="https://wa.me/90XXXXXXXXXX">
                        </div>
                        
                        <div class="ald-form-group">
                            <label class="ald-form-label">E-posta Adresi:</label>
                            <input type="email" name="frontend_dev_email" class="ald-form-control" value="<?php echo esc_attr($frontend_dev_email); ?>" placeholder="info@firmaniz.com">
                        </div>
                        
                        <button type="submit" name="save_frontend_settings" class="ald-btn ald-btn-primary">
                            💾 Ayarları Kaydet
                        </button>
                    </form>
                </div>
            ...
            <div class="ald-card">
                ...
                <div class="ald-card-body">
                    <h4>Müşteriler şu bilgileri görecek:</h4>
                    <?php if ($frontend_show_custom && !empty($frontend_dev_name)): ?>
                        <div style="background: #f8f9fa; padding: 15px; border-radius: 8px; border-left: 4px solid #007cba;">
                            <strong>👨‍💻 Geliştirici:</strong> <?php echo esc_html($frontend_dev_name); ?><br>
                            <?php if (!empty($frontend_dev_phone)): ?>
                                📱 <strong>Telefon:</strong> <?php echo esc_html($frontend_dev_phone); ?><br>
                            <?php endif; ?>
                            <?php if (!empty($frontend_dev_email)): ?>
                                📧 <strong>E-posta:</strong> <?php echo esc_html($frontend_dev_email); ?><br>
                            <?php endif; ?>
                            <?php if (!empty($frontend_dev_whatsapp)): ?>
                                💬 <a href="<?php echo esc_url($frontend_dev_whatsapp); ?>" target="_blank">WhatsApp ile İletişim</a>
                            <?php endif; ?>
                        </div>
                    <?php else: ?>
                        <div style="background: #e3f2fd; padding: 15px; border-radius: 8px; border-left: 4px solid #1976d2;">
                            <strong>👨‍💻 Geliştirici:</strong> BERAT K<br>
                            📱 <strong>WhatsApp:</strong> <a href="https://wa.me/905395115632" target="_blank">0539 511 56 32</a>
                        </div>
                    <?php endif; ?>
                </div>
            </div>
            
            <div style="background: #f0f8ff; padding: 20px; border-radius: 8px; margin-top: 30px; text-align: center; border: 2px solid #90caf9;">
                <h3 style="color: #1976d2; margin: 0 0 10px 0;">🔒 Lisans ve Güvenlik</h3>
                <p style="margin: 0; color: #666;">Bu eklenti <strong>BERAT K</strong> tarafından geliştirilmiştir. Admin paneli, kod yapısı ve lisans hakları korunmaktadır.</p>
                <a href="https://wa.me/905395115632" target="_blank" style="display: inline-block; margin-top: 10px; padding: 8px 16px; background: #25d366; color: white; text-decoration: none; border-radius: 5px;">
                    📱 BERAT K ile İletişim
                </a>
            </div>
        </div>
        <?php
    ...
    private function send_pending_email($order, $product) {
        ...
        // Özelleştirilebilir geliştirici bilgisini al
        $developer_info_html = $this->get_frontend_developer_info('email');
        ...
        ', $customer_name, $product->get_name(), $developer_info_html, get_bloginfo('name'));
    ...
    private function send_license_email($order, $product, $license_key) {
        ...
        ', $customer_name, $product->get_name(), $license_key, $developer_info_html, get_bloginfo('name'));
    ...
    public function license_keys_content() {
        ...
        // Özelleştirilebilir frontend geliştirici bilgisi
        echo $this->get_frontend_developer_info('footer');
    ...
    public function add_license_to_email($order, $sent_to_admin, $plain_text) {
        ...
        if (!empty($licenses)) {
            if ($plain_text) {
                ...
                // Özelleştirilebilir geliştirici bilgisi (plain text)
                $show_custom = get_option('ald_frontend_show_custom', 0);
                $custom_name = get_option('ald_frontend_dev_name', '');
                
                if ($show_custom && !empty($custom_name)) {
                    $dev_phone = get_option('ald_frontend_dev_phone', '');
                    echo "\n👨‍💻 Geliştirici: " . $custom_name;
                    if (!empty($dev_phone)) {
                        echo " - Telefon: " . $dev_phone;
                    }
                    echo "\n";
                } else {
                ...
                echo '</tbody>';
                echo '</table>';
                echo $this->get_frontend_developer_info('email');
            }
        }
    }
    ...
    /**
     * Frontend için özelleştirilebilir geliştirici bilgisi
     */
    private function get_frontend_developer_info($style = 'default') {
        $show_custom = get_option('ald_frontend_show_custom', 0);
        $custom_name = get_option('ald_frontend_dev_name', '');
        
        if ($show_custom && !empty($custom_name)) {
            // Özel geliştirici bilgilerini kullan
            $dev_info = array(
                'name' => get_option('ald_frontend_dev_name', ''),
                'phone' => get_option('ald_frontend_dev_phone', ''),
                'whatsapp' => get_option('ald_frontend_dev_whatsapp', ''),
                'email' => get_option('ald_frontend_dev_email', '')
            );
            
            return $this->render_custom_developer_info($dev_info, $style);
        } else {
            // BERAT K bilgilerini kullan (varsayılan)
            return $this->render_default_developer_info($style);
        }
    }
    
    /**
     * Özel geliştirici bilgisi render
     */
    private function render_custom_developer_info($dev_info, $style = 'default') {
        switch ($style) {
            case 'footer':
                return '<div class="ald-developer-footer">
                    <h4 style="margin: 0 0 10px 0; color: #1976d2;">👨‍💻 Geliştirici: ' . esc_html($dev_info['name']) . '</h4>
                    <p style="margin: 0 0 15px 0; color: #666; font-size: 14px;">Bu eklenti ' . esc_html($dev_info['name']) . ' tarafından özelleştirilmiştir.</p>
                    ' . (!empty($dev_info['whatsapp']) ? '<a href="' . esc_url($dev_info['whatsapp']) . '" class="ald-whatsapp-btn" target="_blank">📱 WhatsApp ile İletişim: ' . esc_html($dev_info['phone']) . '</a>' : '') . '
                    ' . (!empty($dev_info['email']) ? '<div style="margin-top: 10px;"><a href="mailto:' . esc_attr($dev_info['email']) . '" style="color: #666; font-size: 14px;">📧 ' . esc_html($dev_info['email']) . '</a></div>' : '') . '
                </div>';
                
            case 'email':
                return '<div style="margin-top: 20px; padding: 15px; background: #f0f8ff; border-radius: 8px; text-align: center;">
                    <small style="color: #666;">👨‍💻 <strong>Geliştirici:</strong> ' . esc_html($dev_info['name']) . 
                    (!empty($dev_info['phone']) ? ' - 📱 ' . esc_html($dev_info['phone']) : '') .
                    (!empty($dev_info['email']) ? ' - 📧 ' . esc_html($dev_info['email']) : '') . '</small>
                </div>';
                
            default:
                return '<small style="color: #666;">👨‍💻 <strong>Geliştirici:</strong> ' . esc_html($dev_info['name']) . 
                (!empty($dev_info['phone']) ? ' - 📱 ' . esc_html($dev_info['phone']) : '') . '</small>';
        }
    }
    
    /**
     * Varsayılan BERAT K geliştirici bilgisi
     */
    private function render_default_developer_info($style = 'default') {
        switch ($style) {
            case 'footer':
                return '<div class="ald-developer-footer">
                    <h4 style="margin: 0 0 10px 0; color: #1976d2;">👨‍💻 Geliştirici: BERAT K</h4>
                    <p style="margin: 0 0 15px 0; color: #666; font-size: 14px;">Bu eklenti BERAT K tarafından özel olarak geliştirilmiştir.</p>
                    <a href="https://wa.me/905395115632" class="ald-whatsapp-btn" target="_blank">📱 WhatsApp ile İletişim: 0539 511 56 32</a>
                </div>';
                
            case 'email':
                return '<div style="margin-top: 20px; padding: 15px; background: #f0f8ff; border-radius: 8px; text-align: center;">
                    <small style="color: #666;">👨‍💻 <strong>Geliştirici:</strong> BERAT K - WhatsApp: <a href="https://wa.me/905395115632" style="color: #25d366; text-decoration: none;">0539 511 56 32</a></small>
                </div>';
                
            default:
                return '<small style="color: #666;">👨‍💻 <strong>Geliştirici:</strong> BERAT K - WhatsApp: <a href="https://wa.me/905395115632" style="color: #25d366; text-decoration: none;">0539 511 56 32</a></small>';
        }
    }
}
